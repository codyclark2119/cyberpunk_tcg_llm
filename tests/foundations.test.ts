...
import { validateState, listLegalActions, applyAction } from "@tcg/engine";
...
import { fixtureContext, fixtureState, player } from "./contract-fixture";
import { applyCommand } from "@tcg/engine";
const context = fixtureContext();
const state = fixtureState(context);
const action = GameActionSchema.parse({ actorId: player, action: { kind: "PASS_REACT" } });
test("canonical JSON and SHA-256 are stable across nested key insertion order", () => {
    ...
    for (const value of [NaN, Infinity, undefined, new Date(), { x: undefined }, new Array(2)])
        assert.throws(() => canonicalSerialize(value));
    const cyclic: Record<string, unknown> = {};
    cyclic.self = cyclic;
    assert.throws(() => canonicalSerialize(cyclic));
});
...
test("engine is deterministic, preserves state, checks versions and invents no actions", () => {
    ...
    assert.equal(listLegalActions(state, state.timing.actingPlayer, context).ok, true);
    ...
    const stale = applyCommand(state, { ...action, commandId: "00000000-0000-4000-8000-000000000003", idempotencyKey: "test", expectedStateVersion: 1 }, context);
...
test("opaque cursors round-trip, reject bad encoding and bind to filters", async () => {
    const id = CardIdSchema.parse("dev-legend-red");
    const cursor = encodeCursor(id, { search: "red" });
    assert.equal(decodeCursor(cursor, { search: "red" }), id);
    assert.throws(() => decodeCursor(cursor, { search: "blue" }));
    for (const invalid of ["!", "e30", "", cursor + "="])
        assert.throws(() => decodeCursor(invalid));
    assert.equal(PageInputSchema.safeParse({ first: 0 }).success, false);
    const repo = new FixtureCardRepository();
    const page1 = await repo.list({ first: 2 });
    const page2 = await repo.list({ first: 2, afterId: page1.cards[1].id });
...
test("training positions export deterministic deduplicated JSONL and reject corruption", () => {
    const position = generatePosition(state, state.timing.actingPlayer, context, "fixture-position");
