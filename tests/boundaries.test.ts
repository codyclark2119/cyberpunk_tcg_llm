...
const allowed: Record<string, string[]> = {
    wire: ["@tcg/domain", "@tcg/engine", "zod"], domain: ["zod"], engine: ["@tcg/domain", "zod"],
...
test("workspace manifests and source imports enforce dependency direction", () => {
    for (const [name, dependencies] of Object.entries(allowed)) {
        ...
        for (const dependency of Object.keys(manifest.dependencies))
            assert.ok(dependencies.includes(dependency), `${name} -> ${dependency}`);
            ...
            function check(node: ts.Node) {
                ...
                if (specifier) {
                    if (specifier.startsWith("."))
                        assert.ok(!relative(root, resolve(file, "..", specifier)).startsWith(".."), `${file} escapes package`);
                    else {
...
test("offline worker bootstrap imports only headless contracts and Node transport utilities", () => {
    for (const file of ["scripts/engine-worker.ts", "scripts/engine-identity.ts"]) {
        const ast = ts.createSourceFile(file, readFileSync(file, "utf8"), ts.ScriptTarget.Latest, true);
        const permitted = ["node:readline", "node:fs", "node:path", "node:url", "@tcg/domain", "@tcg/wire", "./engine-identity"];
        function inspect(node: ts.Node) {
            if (ts.isImportDeclaration(node) && ts.isStringLiteral(node.moduleSpecifier))
                assert.ok(permitted.includes(node.moduleSpecifier.text), `${file}: forbidden worker import`);
            if (ts.isCallExpression(node) && (node.expression.kind === ts.SyntaxKind.ImportKeyword || (ts.isIdentifier(node.expression) && node.expression.text === "require")))
                assert.fail("Worker dynamic dependencies forbidden");
