...
export function reviewedReplay() {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const selected = state.setup ? legalActions.find(a => a.action.kind === "CHOOSE" && a.action.optionIndices[0] === (state.setup!.stage === "MULLIGAN" && state.setup!.completed === 0 ? 1 : 0))!
            : state.timing.step === "CHOOSE_GIG" ? legalActions[0]
            : state.resolution.searchContinuation ? legalActions.find(a => a.action.kind === "CHOOSE" && state.resolution.choice!.options[a.action.optionIndices[0]].kind === "CARD")!
            : legalActions.find(a => a.action.kind === "CALL_LEGEND" && state.objects.cards[a.action.cardInstanceId].cardId === "viktor-vektor-sit-down-and-relax")!;
