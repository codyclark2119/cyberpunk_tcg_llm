...
export function validateTriggerState(state: GameState, context: EngineContext) {
    ...
    if (!c) return all.some(e => e.trigger || e.primitiveIndex !== undefined) || state.timing.combat.stage === "TRIGGER_RESOLUTION" || ["DISCARD_SELECTION", "TRIGGER_ORDER_SELECTION", "OPTIONAL_TRIGGER_SELECTION"].includes(state.timing.step ?? "") ? failure("ORPHAN_TRIGGER", "Pending trigger metadata requires its continuation") : success(null);
    ...
    for (const e of all) {
        if (e.primitiveIndex !== undefined && (e !== r.current || e.primitiveIndex !== 1 || !supportsOrderedAttackCard(cardRevision(state, e.sourceId!, context), context).ok)) return failure("INVALID_PRIMITIVE_INDEX", "Only the current reviewed ordered ability may advance to its second primitive");
    ...
    if (c.phase !== "SELECT" && (!r.current || !["DISCARD_CARDS", "ADJUST_GIG_UP_TO", "OPTIONAL_DECREASE_FRIENDLY_GIG_THEN_DRAW_IF_MIN", "LOOK_AT_FRIENDLY_FACE_DOWN_LEGEND"].includes(r.current.effect.kind))) return failure("INVALID_TRIGGER_PHASE", "Only reviewed adjustment/look primitives pause for choices");
    ...
    const expectedStep = c.phase === "DISCARD" ? "DISCARD_SELECTION" : c.phase === "SELECT" ? "TRIGGER_ORDER_SELECTION" : c.phase === "OPTIONAL" ? "OPTIONAL_TRIGGER_SELECTION" : c.phase === "TARGET" ? "TARGET_SELECTION" : "AMOUNT_SELECTION";
