...
export function supportsReactPlay(card: DeepReadonly<CardRevisionSnapshot> | undefined, context: EngineContext) {
    if (!reactEnabled(context) || !card || card.execution?.scope !== "COMBAT_REACT_V1" || card.execution.status !== "SUPPORTED" || card.printedCost.kind !== "EDDIES" || card.mechanics.equip || card.mechanics.modifiers.length)
