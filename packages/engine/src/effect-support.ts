...
export function supportsCall(card: DeepReadonly<CardRevisionSnapshot> | undefined, context: EngineContext) {
    ...
    if (!card || (reviewed && card.execution?.status !== "SUPPORTED") || card.execution?.status === "UNSUPPORTED" || card.mechanics.abilities.length > 1 || card.mechanics.abilities.some(a => a.trigger !== "WHEN_CALLED") || card.mechanics.modifiers.some(m => !reviewed || m.kind !== "POWER_PER_EQUIPPED_GEAR_DURING_OWN_TURN"))
    ...
    if (ability && (ability.cost.kind !== "NONE" || ability.conditions.length || ability.effects.length !== 1 || !(ability.effects[0].kind === "DRAW" || (reviewed && ability.effects[0].kind === "SEARCH_GEAR"))))
