...
const visibleCard = z.strictObject({ publicId: z.string(), zone: ZoneSchema, ownerSeat: z.number().int(), controllerSeat: z.number().int(), face: z.enum(["UP", "DOWN"]), readiness: z.enum(["READY", "SPENT"]), lagging: z.boolean().optional(), attachments: z.array(z.string()).optional(), effectivePower: z.number().int().nullable().optional(), content: CardReferenceSchema.optional(), damage: z.number().int(), counters: z.record(z.string(), z.number().int()) });
export const PlayerObservationSchema = z.strictObject({ schemaVersion: z.literal(1), viewerSeat: z.number().int(), turn: z.number().int(), activeSeat: z.number().int(), actingSeat: z.number().int(), window: z.enum(["SETUP", "MAIN", "CHOOSE_GIG", "PAYMENT_SELECTION", "TARGET_SELECTION", "AMOUNT_SELECTION", "ATTACK_TARGET_SELECTION", "RIVAL_REACT", "COMBAT_RESOLUTION_PENDING", "RESOLVING", "FINISHED"]), resolutionStage: z.string(), step: TurnStepSchema.optional(), outcome: z.strictObject({ winnerSeat: z.number().int(), reason: z.enum(["EMPTY_DRAW", "START_TURN_GIGS"]) }).optional(),
    combat: z.strictObject({ stage: z.enum(["ATTACK_TARGET_SELECTION", "RIVAL_REACT", "COMBAT_RESOLUTION_PENDING"]), attackerId: z.string(), attackingSeat: z.number().int(), defendingSeat: z.number().int(), target: z.discriminatedUnion("kind", [z.strictObject({ kind: z.literal("CARD"), cardInstanceId: z.string() }), z.strictObject({ kind: z.literal("GIG_AREA"), playerSeat: z.number().int() })]).nullable() }).optional(),
...
export function observe(state: GameState, actor: PlayerId, context: EngineContext) {
    ...
    const players = state.match.playerOrder.map(id => {
        ...
        const cards = Object.entries(p.zones).flatMap(([zone, refs]) => {
            ...
            return (refs ?? []).flatMap((cid, index) => {
                ...
                return { publicId: known ? cid : `seat:${p.seat}:${zone}:${index}`, zone: c.zone.zone, ownerSeat: seat(c.ownerId), controllerSeat: seat(c.controllerId), face: c.face, readiness: c.readiness, ...(c.statuses.includes("LAG") ? { lagging: true } : {}), ...(gearEnabled(context) && c.face === "UP" && ["BATTLEFIELD", "LEGENDS"].includes(zone) ? { attachments: view.getAttachedGear(cid).map(g => g.id), effectivePower: view.getEffectivePower(cid) } : {}), ...(known ? { content: { cardId: c.cardId, revision: c.revision } } : {}), damage: c.damage, counters: c.counters };
            });
        });
    ...
    const combatObservation = combat.stage === "ATTACK_TARGET_SELECTION" || combat.stage === "RIVAL_REACT" || combat.stage === "COMBAT_RESOLUTION_PENDING" ? { combat: { stage: combat.stage, attackerId: combat.attackerId, attackingSeat: seat(combat.attackingPlayerId), defendingSeat: seat(state.match.playerOrder.find(id => id !== combat.attackingPlayerId)!), target: combat.target?.kind === "GIG_AREA" ? { kind: "GIG_AREA", playerSeat: seat(combat.target.playerId) } : combat.target }, ...(combat.stage === "COMBAT_RESOLUTION_PENDING" ? { unsupportedCapabilities: ["UNSUPPORTED_COMBAT_RESOLUTION"] } : combat.stage === "RIVAL_REACT" && !reactEnabled(context) ? { unsupportedCapabilities: ["UNSUPPORTED_RIVAL_REACT"] } : {}) } : {};
