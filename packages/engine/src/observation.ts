...
export const PlayerObservationSchema = z.strictObject({ schemaVersion: z.literal(1), viewerSeat: z.number().int(), turn: z.number().int(), activeSeat: z.number().int(), actingSeat: z.number().int(), window: z.enum(["SETUP", "MAIN", "CHOOSE_GIG", "PAYMENT_SELECTION", "TARGET_SELECTION", "RIVAL_REACT", "RESOLVING", "FINISHED"]), resolutionStage: z.string(), step: TurnStepSchema.optional(), outcome: z.strictObject({ winnerSeat: z.number().int(), reason: z.enum(["EMPTY_DRAW", "START_TURN_GIGS"]) }).optional(),
