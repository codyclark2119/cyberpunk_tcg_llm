...
export function effectiveTriggeredAbilities(state: GameState, subjectId: CardInstanceId, context: EngineContext): TriggerBinding[] {
    ...
    return sources.flatMap(source => {
        ...
        if (!supportsTriggerCard(r, context).ok || !r) return [];
...
export function triggerChoice(state: GameState): PendingChoice {
    ...
    else if (c.phase === "TARGET") { kind = "TARGET"; options = triggerGigTargets(state).map(gigInstanceId => ({ kind: "GIG", gigInstanceId })); }
    else {
