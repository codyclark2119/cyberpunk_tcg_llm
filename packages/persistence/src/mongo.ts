...
export function connectMongo(env: Environment): {
    db: () => Promise<Db>;
    close: () => Promise<void>;
} {
...
export function cardFromDocument(raw: unknown): Card {
    ...
    if (doc.cardId !== doc.content.id || doc.revision !== doc.content.revision || doc.contentHash !== hashCardContent(doc.content))
        throw new Error("Corrupt card snapshot identity/hash");
    return doc.content;
}
...
export async function createMongoIndexes(db: Db): Promise<void> {
    ...
    // Forward migration: collector numbers repeat across products/cards. Stable _id owns identity.
    if ((await db.listCollections({ name: "cards" }).toArray()).length && await cards.indexExists("index.cardNumber_1")) {
        const old = (await cards.indexes()).find(i => i.name === "index.cardNumber_1");
        if (old?.unique)
            await cards.dropIndex("index.cardNumber_1");
    }
    await cards.createIndex({ "index.cardNumber": 1 });
...
export class MongoCardRepository implements CardRepository {
    ...
    async list({ first, afterId, filter }: CardPageRequest) {
        ...
        try {
            ...
            if (cause instanceof MongoServerError && (cause.code === 27 || cause.code === 26))
                throw new Error("Mongo card search indexes are missing. Run npm run mongo:indexes against the app's configured Mongo database.", { cause });
            throw cause;
        }
    }
    async findById(id: CardId) {
        const db = await this.getDb();
        const raw = await db.collection<CardDocument>("cards").findOne({ _id: id });
        return raw ? cardFromDocument(raw) : null;
    }
    async findByIds(ids: CardId[]) {
        const db = await this.getDb();
        return (await db.collection<CardDocument>("cards").find({ _id: { $in: ids } }).toArray()).map(cardFromDocument);
    }
    async findRevision(cardId: CardId, revision: CardRevision) {
        const db = await this.getDb();
        const raw = await db.collection<CardDocument>("card_revisions").findOne({ cardId, revision });
        return raw ? cardFromDocument(raw) : null;
    }
}
export class MongoRulesetRepository implements RulesetRepository {
    ...
    async findVersion(id: RulesetId, version: RulesetVersion): Promise<Ruleset | null> {
        const db = await this.getDb();
        const raw = await db.collection("ruleset_revisions").findOne({ rulesetId: id, version });
        if (!raw)
            return null;
        const doc = RulesetDocumentSchema.parse(raw);
        if (doc.rulesetId !== doc.content.id || doc.version !== doc.content.version || doc.contentHash !== hashRulesetContent(doc.content))
            throw new Error("Corrupt ruleset snapshot identity/hash");
        return doc.content;
    }
    async publish(ruleset: Ruleset): Promise<PublishResult> {
        const db = await this.getDb();
        const contentHash = hashRulesetContent(ruleset);
        try {
