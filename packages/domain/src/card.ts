import { EquipTargetSchema, CostSchema, KeywordSchema, AbilitySchema, ContinuousModifierSchema } from "./mechanics";
import { z } from "zod";
...
export const CardRevisionSnapshotSchema = LegacyCardSchema.extend({
    ...
    execution: z.strictObject({ scope: z.enum(["NONCOMBAT_SLICE_V1", "NONCOMBAT_PLAY_V1", "COMBAT_ATTACK_V1", "COMBAT_REACT_V1"]), status: z.enum(["SUPPORTED", "UNSUPPORTED"]) }).optional(),
    mechanics: z.strictObject({ equip: EquipTargetSchema.optional(), keywords: z.array(KeywordSchema), abilities: z.array(AbilitySchema), modifiers: z.array(ContinuousModifierSchema) }),
