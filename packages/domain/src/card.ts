import { CostSchema, KeywordSchema, AbilitySchema, ContinuousModifierSchema } from "./mechanics";
import { z } from "zod";
...
export const CardRevisionSnapshotSchema = LegacyCardSchema.extend({
    ...
    execution: z.strictObject({ scope: z.literal("NONCOMBAT_SLICE_V1"), status: z.enum(["SUPPORTED", "UNSUPPORTED"]) }).optional(),
    mechanics: z.strictObject({ keywords: z.array(KeywordSchema), abilities: z.array(AbilitySchema), modifiers: z.array(ContinuousModifierSchema) }),
