...
This is the implementation contract for Batch V5. It incorporates the review of `0cf8619463958e3e31e42ad87e6feb37feda9e51`, based on post-V4 master `3efe0f72b0f8f04c1268b87633fbba4fdf7c269f`. **PR #10 has since merged at `f4562acd84e6e4f31dee0285fe97f9eaa6fe4ee8`**, delivering the runtime Gear-defeat capability described below.

Both runtime slices are now merged in PR #10: the Gear vocabulary and reserved scope/policy, and the executing capability (selector, metadata ownership split and registration, defeat/departure opt-in, and Main plus bounded defender-React continuation validation). Runtime identity is `0.4.0-api-admission-5`, artifact hash `e5bcf81771ad6430ad8bc16d5f8508b35c0d77e3dc96b6f6264ddea4e2366c06`. The follow-on branch `review/api-admission-batch-v5-detonate` adds the real Detonate revision, its pinned source evidence, and legal Main/React replay coverage, and changes no hashed runtime input. **Still pending:** the full regeneration chain, matrix, Descriptor V2 corpus, preserved-replay compatibility, unit/integration and build gates. See `api-admission-batch-v5-implementation-status.md` for the precise merged/pending split.
...
Use the separate execution scope and opt-in policy `TARGETED_GEAR_DEFEAT_V1` (policy field `targetedGearDefeat`). As merged, `supportsPlay` and `supportsReactPlay` delegate the new scope to `supportsTargetedGearDefeatCard`, while Floor It's exact React branch and the two-card Unit gate are untouched. The two-card acceptance predicate of `TARGETED_DEFEAT_V1` was never extended to allow Quick.
...
The registration point is **`validateState` in `packages/engine/src/state.ts`**, next to the existing `validateTargetedDefeatMetadata` call. As merged, `validateTargetedGearDefeatMetadata` is registered there and the old detector is partitioned: it owns `TARGETED_DEFEAT_V1` and Unit-target effects only, while the Gear detector owns the Gear scope and Gear-target effects. A mixed card is claimed by both and must satisfy both complete gates, so nothing falls between them. Direct hidden-state reload tests pin that fail-closed behaviour.
...
PR #10 merged once the capability, its React validation and old admission isolation were proven. The follow-on Detonate PR adds the pinned evidence, the explicit immutable revision, the honestly recorded destination decision, and both-area/own-power tests, and must remain draft until generated outputs are reproducible and preserved replays, matrix, Descriptor V2, full tests, configured integration, card validation, build and diff checks all pass.

History: the first slice implemented vocabulary and isolated shape review with execution deliberately closed; the second opened execution and added the continuation validation; both merged in PR #10 with their focused suites and mutation checks passing. The Detonate follow-up adds evidence capture and legal replay. Regeneration, matrix, Descriptor V2 corpus and full-repository gates remain outstanding and are not claimed completed.
