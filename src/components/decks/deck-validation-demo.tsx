...
import { VALIDATE_DECK } from "@/graphql/operations";

type RamValue = { color: string; amount: number };
type ValidationIssue = { code: string; message: string; severity: string; cardId?: string | null };
...
export function DeckValidationDemo() {
  const [validateDeck, { data, loading, error }] = useMutation<
    { validateDeck: ValidationResult },
    { input: typeof developmentDeck }
  >(VALIDATE_DECK);
